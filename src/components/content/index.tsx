...
const Content = forwardRef<ContentRef, ContentProps>(
  ({ value, taggedUsers, className, style = {} }, ref) => {
    const handleClickUserTag = async (
        ...
        <>
          {tokenizedContent.map((subString, i) => {
            return isUsername(subString) ? (
              ...
              <span key={i} className="TroveContent__Substring">
                {subString}
              </span>
            );
          })}
    ...
    return (
      <div className={`TroveExtension__EditorContent ${className}`} ref={ref} style={style}>
